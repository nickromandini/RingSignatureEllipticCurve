...
const ring_signature_hex = (signing_key, key_idx, M, y) => {
    ...
    for(var i = 0 ; i<y.length; i++) {
        ...
        //pubkeys_points_strings.push([pubkeys_as_points[i].affineX.toString(), pubkeys_as_points[i].affineY.toString()])
    }

    signature = ring_signature(signing_key_int, key_idx, M, pubkeys_as_points)
    result = {}
    result["c_0"] = signature[0].toString()
    s = []
    for(var i = 0; i < signature[1].length; i++) {
        s.push(signature[1][i].toString())
    }
    result["s"] = s
    result["Y"] = [signature[2].affineX.toString(),signature[2].affineY.toString()]
...
let positionPKSigner = Math.floor(Math.random() * numkeys)

let keys = new Array(numkeys)
let businessesKeys = new Array(numkeys)
for(i=0;i<numkeys;i++) {
    keys[i] = ec.genKeyPair()
    businessesKeys[i] = keys[i].getPublic().encode('hex').slice(0,66)
}

var fs = require('fs');


fs.appendFile('keys.json', JSON.stringify(businessesKeys) , function (err) {
  if (err) throw err;
});
message = "hello"
res = ring_signature_hex(keys[positionPKSigner].getPrivate().toString('hex'), positionPKSigner, message, businessesKeys)

fs.appendFile('res.json', JSON.stringify(res) , function (err) {
    if (err) throw err;
});
